...
type XMLDMXMode struct {
	Name        string           `xml:",attr"`
	Description string           `xml:",attr"`
	Geometry    XMLNodeReference `xml:",attr"`
	DMXChannels []XMLDMXChannel  `xml:"DMXChannels>DMXChannel"`
	Relations   []XMLRelation    `xml:"Relations>Relation"`
	FTMacros    []XMLFTMacro     `xml:"FTMacros>FTMacro"`
}

type XMLDMXChannel struct {
	DMXBreak        int                 `xml:",attr"`
	Offset          []int               `xml:",attr"`
	InitialFunction XMLNodeReference    `xml:",attr"`
	Highlight       XMLDMXValue         `xml:",attr"`
	Geometry        XMLNodeReference    `xml:",attr"`
	LogicalChannels []XMLLogicalChannel `xml:"LogicalChannel"`
}
...
type XMLChannelFunction struct {
	Name              string             `xml:",attr"`
	Attribute         XMLNodeReference   `xml:",attr"`
	...
	ChannelSets       []XMLChannelSet    `xml:"ChannelSet"`
	SubChannelSets    []XMLSubChannelSet `xml:"SubChannelSet"`
}

type XMLChannelSet struct {
	Name           string      `xml:",attr"`
	DMXFrom        XMLDMXValue `xml:",attr"`
	PhysicalFrom   float32     `xml:",attr"`
	PhysicalTo     float32     `xml:",attr"`
